...
def main():
    ...
    with open('makers.csv') as csvfile:
        ...
        for row in reader:
            ...
            web_address = row['{website}'].strip().replace('http://','').replace('https://','').replace('www.','').strip()
            ...
            for para in root.findall('.//{http://www.w3.org/2000/svg}flowPara'):
                if para.text == '{title}':
                    ...
                    if len(name) >= 34:
                        ...
                        print('name font-size: ' + str(font_size) + ' px;')
                        #print(parent.attrib['style'])
                elif para.text == '{location}':
                    para.text = location
                    ...
                    if web_address[-1:] == '/':
                        para.text = web_address[:-1]
                    else:
                        para.text = web_address
